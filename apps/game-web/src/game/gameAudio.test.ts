...
describe("game audio controller", () => {
  ...
  it("plays a brighter confirmation tone for clean hazard escapes", () => {
    const context = new FakeAudioContext();
    const controller = createGameAudioController({ createContext: () => context });

    controller.play(["clean-escape"]);

    expect(context.oscillators).toHaveLength(1);
    expect(context.oscillators[0]?.frequencyValue).toBe(940);
  ...
  it("plays a confident route-launch confirmation tone", () => {
    ...
    controller.play(["route-launch"]);

    expect(context.oscillators).toHaveLength(1);
    expect(context.oscillators[0]?.frequencyValue).toBe(640);
    expect(context.oscillators[0]?.stoppedAt).toBeCloseTo(0.15, 3);
  });

  it("plays a tight route-resume confirmation tone", () => {
    const context = new FakeAudioContext();
    const controller = createGameAudioController({ createContext: () => context });

    controller.play(["route-resume"]);
  ...
  it("plays a clear arming tone for antimatter drift windows", () => {
    const context = new FakeAudioContext();
    const controller = createGameAudioController({ createContext: () => context });

    controller.play(["antimatter-armed"]);

    expect(context.oscillators).toHaveLength(1);
    expect(context.oscillators[0]?.frequencyValue).toBe(1060);
    expect(context.oscillators[0]?.stoppedAt).toBeCloseTo(0.11, 3);
  });
