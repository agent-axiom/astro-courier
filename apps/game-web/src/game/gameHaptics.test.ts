...
describe("game haptics", () => {
  it("maps gameplay event batches to tactile patterns by priority", () => {
    ...
    expect(hapticPatternForEvents(["route-launch"])).toEqual([16, 20, 16]);
    expect(hapticPatternForEvents(["route-resume"])).toEqual([8, 10]);
    expect(hapticPatternForEvents(["tempo-flow"])).toEqual([8, 8, 12]);
    expect(hapticPatternForEvents(["tempo-clutch"])).toEqual([10, 16, 10]);
    expect(hapticPatternForEvents(["combo-burst"])).toEqual([14, 12, 22]);
    ...
    expect(hapticPatternForEvents(["last-drop-armed"])).toEqual([18, 18, 28]);
    expect(hapticPatternForEvents(["chain-critical"])).toEqual([12, 24, 12, 24]);
    ...
    expect(hapticPatternForEvents(["clean-escape"])).toEqual([10, 10, 18]);
